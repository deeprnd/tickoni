...
test "binary and wire format pinned" { if (std.c.getenv("TK_GEN_FIXTURES") != null) return error.SkipZigTest;
...
fn cwrite(f: *std.c.FILE, s: []const u8) void { _ = std.c.fwrite(s.ptr, 1, s.len, f); }
...
test "gen audit fixture values" { if (std.c.getenv("TK_GEN_FIXTURES") == null) return error.SkipZigTest;
