...
const default_quotes: [basket.max_basket_instruments]trade_ticket.Quote = blk: {
    var arr = std.mem.zeroes([basket.max_basket_instruments]trade_ticket.Quote);
...
const QuotesFileWire = struct {
    as_of_ns: u64,
    quotes: []const QuoteWireEntry,
};

const PaperFillWire = struct {
    ticker: []const u8,
    quantity: []const u8,
...
const PaperExecutionFileWire = struct {
...
fn parseVenue(s: []const u8) !@FieldType(trade_ticket.Quote, "venue") {
    if (std.mem.eql(u8, s, "NASDAQ")) return .nasdaq;
...
fn loadAccountFromDir(
    ...
    return portfolio.BrokerageAccount{
        .account_id = w.numeric_account_id,
        .currency = .usd,
...
fn loadQuoteLoaderFromDir(
    ...
    var loader = QuoteLoader{
        .as_of_ns = wire.as_of_ns,
        .quote_count = @intCast(wire.quotes.len),
        .quotes = std.mem.zeroes([basket.max_basket_instruments]trade_ticket.Quote),
    };
    for (wire.quotes, 0..) |wq, i| {
        if (wq.ticker.len > portfolio.max_ticker_len) return error.TickerTooLong;
...
fn loadPaperExecutionFromDir(
    ...
    for (w.fills, 0..) |wf, i| {
        if (wf.ticker.len > portfolio.max_ticker_len) return error.TickerTooLong;
...
test "FixtureBackend.initFromDir data matches hardcoded defaults" {
    ...
    for (0..from_default.quote_loader.quote_count) |i| {
        const dq = from_default.quote_loader.quotes[i];
