...
fn nameBuf(comptime s: []const u8) [max_name_len]u8 { if (s.len > max_name_len) @compileError("name exceeds max_name_len");
    var buf = std.mem.zeroes([max_name_len]u8);
    for (s, 0..) |byte, i| buf[i] = byte;
    return buf;
}

fn sectorRef(code: []const u8) ClassificationRef { return thesis.ClassificationRef.init("gics_sector", sector_taxonomy_version, code) catch unreachable; }

fn industryRef(code: []const u8) ClassificationRef { return thesis.ClassificationRef.init("gics_industry", industry_taxonomy_version, code) catch unreachable; }

fn sectors(codes: []const []const u8) ClassificationRefList {
    var refs = ClassificationRefList{};
    for (codes) |code| { refs.append(sectorRef(code)) catch unreachable; }
    return refs;
}

fn industries(codes: []const []const u8) ClassificationRefList {
    var refs = ClassificationRefList{};
    for (codes) |code| { refs.append(industryRef(code)) catch unreachable; }
    return refs;
}

fn themes(ids: []const []const u8) ThemeIdList {
    var list = ThemeIdList{};
    for (ids) |id| { list.append(thesis.CanonicalId.init(id) catch unreachable) catch unreachable; }
    return list;
}
...
pub fn validateCatalog() CatalogValidationError!void { for (catalog, 0..) |entry, i| {
...
fn validateEntry(entry: InstrumentEntry) CatalogValidationError!void { if (entry.ticker_len == 0 or entry.ticker_len > max_ticker_len) return CatalogValidationError.InvalidTicker;
    ...
    for (entry.industries.values[0..entry.industries.count]) |industry| { if (!isKnownIndustryRef(industry)) return CatalogValidationError.InvalidClassification; }
    for (entry.themes.values[0..entry.themes.count]) |theme_id| { if (!isKnownThemeId(theme_id)) return CatalogValidationError.InvalidClassification; }
...
pub fn filterByTheme(theme: CanonicalId, out: []*const InstrumentEntry) usize { var n: usize = 0;
    for (0..catalog.len) |i| {
        if (n >= out.len) break;
...
pub fn filterBySector(sector: ClassificationRef, out: []*const InstrumentEntry) usize { var n: usize = 0;
    for (0..catalog.len) |i| {
        if (n >= out.len) break;
        const entry = &catalog[i];
...
pub fn filterByIndustry(industry: ClassificationRef, out: []*const InstrumentEntry) usize { var n: usize = 0;
    for (0..catalog.len) |i| {
        if (n >= out.len) break;
        const entry = &catalog[i];
...
pub fn filterByAssetClass(asset_class: AssetClass, out: []*const InstrumentEntry) usize { var n: usize = 0;
    for (0..catalog.len) |i| {
        if (n >= out.len) break;
        const entry = &catalog[i];
...
pub fn filterByInstrumentType(instrument_type: InstrumentType, out: []*const InstrumentEntry) usize { var n: usize = 0;
    for (0..catalog.len) |i| {
        if (n >= out.len) break;
        const entry = &catalog[i];
...
pub fn filterByVenue(venue: Venue, out: []*const InstrumentEntry) usize { var n: usize = 0;
    for (0..catalog.len) |i| {
        if (n >= out.len) break;
        const entry = &catalog[i];
...
pub fn lookupByTicker(ticker: []const u8) ?*const InstrumentEntry { for (0..catalog.len) |i| {
        const entry = &catalog[i];
        if (std.mem.eql(u8, entry.tickerSlice(), ticker)) return entry; }
    return null;
}

test "catalog validates classification bounds and uniqueness" { try validateCatalog(); }

test "catalog: total entry count is 24" { try std.testing.expectEqual(@as(usize, 24), catalog.len); }

test "catalog: all entries have non-empty ticker and name" { for (catalog) |entry| {
...
test "catalog: restricted entries have non-none restriction_reason" { for (catalog) |entry| {
        if (entry.restricted) {
            try std.testing.expect(entry.restriction_reason != .none); } else { try std.testing.expectEqual(RestrictionReason.none, entry.restriction_reason); }
    }
}

test "catalog: all stocks have expense_ratio_bps == 0" { for (catalog) |entry| {
        if (entry.instrument_type == .stock) {
            try std.testing.expectEqual(@as(u16, 0), entry.expense_ratio_bps); }
    }
}

test "catalog: all entries are US market" { for (catalog) |entry| {
        try std.testing.expectEqual(Market.us, entry.market); }
}

test "catalog: all entries are NYSE or NASDAQ" { for (catalog) |entry| {
        const ok = entry.venue == .nyse or entry.venue == .nasdaq;
        try std.testing.expect(ok); }
}

test "filterByTheme: ai_infrastructure scenario yields >= 4 eligible and >= 2 restricted" { var out: [catalog.len]*const InstrumentEntry = undefined;
...
test "filterByTheme: every returned entry has the requested theme id" { const theme_ids = [_]CanonicalId{
    ...
    for (theme_ids) |theme_id| { const n = filterByTheme(theme_id, &out);
        for (out[0..n]) |entry| {
            try std.testing.expect(entry.themes.has(theme_id)); }
    }
}

test "filterBySector: information technology returns tagged entries" { var out: [catalog.len]*const InstrumentEntry = undefined;
...
test "filterByIndustry: semiconductors returns tagged entries" { var out: [catalog.len]*const InstrumentEntry = undefined;
...
test "catalog: sector and industry refs preserve canonical taxonomy metadata" { const amzn = lookupByTicker("AMZN").?;
    ...
    for (botz.industries.values[0..botz.industries.count]) |industry| { try std.testing.expectEqualStrings("gics_industry", industry.taxonomy_id.slice());
        try std.testing.expectEqual(industry_taxonomy_version, industry.taxonomy_version); }
}

test "filterByAssetClass: cash returns only cash exposures" { var out: [catalog.len]*const InstrumentEntry = undefined;
...
test "filterByInstrumentType: etf returns only ETFs" { var out: [catalog.len]*const InstrumentEntry = undefined;
...
test "filterByInstrumentType: stock returns only stocks" { var out: [catalog.len]*const InstrumentEntry = undefined;
...
test "filterByVenue: every returned entry is on the requested venue" { var nasdaq_out: [catalog.len]*const InstrumentEntry = undefined;
    ...
    for (nyse_out[0..nyse_n]) |entry| { try std.testing.expectEqual(Venue.nyse, entry.venue); }
}

test "lookupByTicker: NVDA returns correct entry" { const entry = lookupByTicker("NVDA");
...
test "lookupByTicker: SOXL is restricted with leveraged_etf reason" { const entry = lookupByTicker("SOXL");
    try std.testing.expect(entry != null);
    try std.testing.expect(entry.?.restricted);
...
test "lookupByTicker: SOXS is restricted with inverse_etf reason" { const entry = lookupByTicker("SOXS");
    try std.testing.expect(entry != null);
    try std.testing.expect(entry.?.restricted);
    try std.testing.expectEqual(RestrictionReason.inverse_etf, entry.?.restriction_reason); }

test "lookupByTicker: BULZ is restricted with leveraged_etf reason" { const entry = lookupByTicker("BULZ");
    try std.testing.expect(entry != null);
    try std.testing.expect(entry.?.restricted);
    try std.testing.expectEqual(RestrictionReason.leveraged_etf, entry.?.restriction_reason); }

test "lookupByTicker: unknown ticker returns null" { try std.testing.expectEqual(@as(?*const InstrumentEntry, null), lookupByTicker("ZZZZ"));
    try std.testing.expectEqual(@as(?*const InstrumentEntry, null), lookupByTicker("")); }

test "lookupByTicker: all catalog tickers are individually resolvable" { for (&catalog) |*entry| {
        const found = lookupByTicker(entry.tickerSlice());
        try std.testing.expect(found != null);
        try std.testing.expectEqualStrings(entry.tickerSlice(), found.?.tickerSlice()); }
}

test "catalog: ticker slices match expected values for known entries" { const nvda = lookupByTicker("NVDA").?;
