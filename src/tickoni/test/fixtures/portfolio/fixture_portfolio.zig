...
fn mkOpenOrder(comptime ticker_s: []const u8, side: Side, notional_cents: i64) OpenOrder {
    return .{
        .ticker = tickerBuf(ticker_s),
        .ticker_len = @intCast(ticker_s.len),
        .side = side,
...
pub const fixtures = struct {
    ...
    pub const low_cash: BrokerageAccount = .{
        .account_id = 2002,
        .currency = .usd,
        .cash_cents = 15_000,
    ...
    pub const diversified: BrokerageAccount = blk: {
        var a = std.mem.zeroes(BrokerageAccount);
        a.account_id = 2004;
        a.currency = .usd;
    ...
    pub const restricted_account: BrokerageAccount = blk: {
        var a = std.mem.zeroes(BrokerageAccount);
        a.account_id = 2005;
        a.currency = .usd;
...
test "portfolio_schema_version is 1" {
    try std.testing.expectEqual(@as(u16, 1), portfolio.portfolio_schema_version);
}

test "fixtures: all account_ids are distinct" {
    const ids = [_]u32{
...
test "fixtures: all accounts use usd currency" {
    for ([_]*const BrokerageAccount{
        &fixtures.cash_rich,
        &fixtures.low_cash,
        &fixtures.technology_heavy,
        &fixtures.diversified,
        &fixtures.restricted_account,
    }) |a| {
        try std.testing.expectEqual(portfolio.Currency.usd, a.currency);
    }
}

test "fixtures: holding_count and open_order_count are in bounds" {
    for ([_]*const BrokerageAccount{
        &fixtures.cash_rich,
        &fixtures.low_cash,
        &fixtures.technology_heavy,
        ...
        try std.testing.expect(a.holding_count <= max_holdings);
        try std.testing.expect(a.open_order_count <= max_snapshot_open_orders);
    }
}

test "fixtures: day_notional_used_cents <= day_notional_limit_cents" {
    for ([_]*const BrokerageAccount{
        &fixtures.cash_rich,
        &fixtures.low_cash,
        &fixtures.technology_heavy,
        ...
        try std.testing.expect(a.day_notional_used_cents <= a.day_notional_limit_cents);
...
test "fixtures: restricted_account day notional is fully used" {
    try std.testing.expectEqual(
...
test "fixtures: cash_rich buying_power covers the USD 2,000 AI thesis target" {
    try std.testing.expect(fixtures.cash_rich.buying_power_cents >= 200_000);
}
...
test "checkAffordability: cash_rich affords AI thesis notional" {
    const result = portfolio.checkAffordability(&fixtures.cash_rich, 200_000);
...
test "checkAffordability: technology_heavy affords USD 2,000 basket" {
    const result = portfolio.checkAffordability(&fixtures.technology_heavy, 200_000);
    try std.testing.expectEqual(AffordabilityOutcome.allow, result.outcome);
}

test "checkAffordability: diversified affords USD 2,000 basket" {
    const result = portfolio.checkAffordability(&fixtures.diversified, 200_000);
    try std.testing.expectEqual(AffordabilityOutcome.allow, result.outcome);
}

test "checkAffordability: low_cash denied for USD 200 request" {
    const result = portfolio.checkAffordability(&fixtures.low_cash, 20_000);
...
test "checkAffordability: deny_insufficient_buying_power when buying_power is zero" {
...
test "checkAffordability: day limit exhausted denies any positive request" {
    var a = fixtures.cash_rich;
...
test "checkAffordability: day limit exceeded by large request" {
    const result = portfolio.checkAffordability(&fixtures.cash_rich, 3_000_000);
    try std.testing.expectEqual(AffordabilityOutcome.deny_day_limit_exceeded, result.outcome);
    try std.testing.expectEqual(@as(i64, 2_500_000), result.max_affordable_cents);
}

test "checkAffordability: allow when request equals remaining daily notional exactly" {
    const result = portfolio.checkAffordability(&fixtures.cash_rich, 2_500_000);
    try std.testing.expectEqual(AffordabilityOutcome.allow, result.outcome);
}

test "checkAffordability: month limit exhausted denies any positive request" {
    var a = fixtures.cash_rich;
...
test "checkAffordability: allow when request equals remaining monthly notional exactly" {
    var a = fixtures.cash_rich;
...
test "checkAffordability: restricted_account denied due to open-order limit" {
    const result = portfolio.checkAffordability(&fixtures.restricted_account, 1);
...
test "checkBasketAffordability: uses basket total_allocated_cents for the request" {
    var proposed_basket = std.mem.zeroes(basket_mod.Basket);
...
test "AffordabilityCheckPayload: fields populate from result correctly" {
    const result = portfolio.checkAffordability(&fixtures.cash_rich, 200_000);
