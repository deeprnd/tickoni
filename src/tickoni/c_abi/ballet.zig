...
pub const Siphash13 = extern struct {
    bytes: [128]u8 align(128) = undefined,
};
...
pub fn siphashInit(sip: *Siphash13, k0: u64, k1: u64) void {
    _ = tk_siphash13_init(sip, k0, k1);
}

pub fn siphashAppend(sip: *Siphash13, data: []const u8) void {
    _ = tk_siphash13_append(sip, data.ptr, data.len);
}

pub fn siphashFini(sip: *Siphash13) u64 {
    return tk_siphash13_fini(sip);
}

test "Siphash13 matches the shim's opaque size and alignment" {
    try std.testing.expectEqual(@as(usize, 128), @sizeOf(Siphash13));
...
pub const PbEncoder = extern struct {
    buf0: ?[*]u8 = null,
    buf1: ?[*]u8 = null,
    cur: ?[*]u8 = null,
    depth: u32 = 0,
...
pub fn pbEncoderInit(encoder: *PbEncoder, out: []u8) bool {
    return tk_pb_encoder_init(encoder, out.ptr, out.len) != null;
}
pub fn pbEncoderFini(encoder: *PbEncoder) bool {
    return tk_pb_encoder_fini(encoder) != 0;
}
pub fn pbEncoderOutSz(encoder: *PbEncoder) u64 {
    return tk_pb_encoder_out_sz(encoder);
}
pub fn pbSubmsgOpen(encoder: *PbEncoder, field_id: u32) bool {
    return tk_pb_submsg_open(encoder, field_id) != 0;
}
pub fn pbSubmsgClose(encoder: *PbEncoder) bool {
    return tk_pb_submsg_close(encoder) != 0;
}
pub fn pbPushUint32(encoder: *PbEncoder, field_id: u32, value: u32) bool {
    return tk_pb_push_uint32(encoder, field_id, value) != 0;
}
pub fn pbPushUint64(encoder: *PbEncoder, field_id: u32, value: u64) bool {
    return tk_pb_push_uint64(encoder, field_id, value) != 0;
}
pub fn pbPushInt64(encoder: *PbEncoder, field_id: u32, value: i64) bool {
    return tk_pb_push_int64(encoder, field_id, value) != 0;
}
pub fn pbPushBytes(encoder: *PbEncoder, field_id: u32, bytes: []const u8) bool {
    return tk_pb_push_bytes(encoder, field_id, bytes.ptr, bytes.len) != 0;
}
pub fn pbInbufInit(buf: *PbInbuf, data: []const u8) void {
    _ = tk_pb_inbuf_init(buf, data.ptr, data.len);
}
pub fn pbInbufSz(buf: *PbInbuf) u64 {
    return tk_pb_inbuf_sz(buf);
}
pub fn pbInbufCur(buf: *PbInbuf) [*]const u8 {
    return tk_pb_inbuf_cur(buf);
}
pub fn pbInbufAdvance(buf: *PbInbuf, sz: u64) void {
    tk_pb_inbuf_advance(buf, sz);
}
pub fn pbReadTlv(buf: *PbInbuf, tlv: *PbTlv) bool {
    return tk_pb_read_tlv(buf, tlv) != 0;
}

test "PbEncoder/PbInbuf/PbTlv match the shim's layout" {
    try std.testing.expectEqual(@as(usize, 8 * 3 + 4 + 4 * pb_encoder_depth_max), @sizeOf(PbEncoder));
