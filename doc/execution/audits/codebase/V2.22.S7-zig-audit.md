...
- Inconsistent with the codebase's own documented pattern, which prescribes `std.mem.sliceTo(v, 0)` for all `getenv` usage
...
**Note:** This is the only remaining Zig 0.15→0.16 migration gap in the codebase. All other 0.15 breaking changes (`std.io`→`std.Io`, `b.host` removal, `addStaticLibrary` removal, `fixedBufferWriter`→`Writer.fixed`, `ArrayList` init changes, `.getWritten()` removal) have already been addressed.
...
| **P1** | `std.c.W.NOHANG` → `std.posix.W.NOHANG` | Trivial (1 char) | Eliminates C FFI, completes 0.15→0.16 migration |
...
**0.15→0.16 migration status:** Complete. All 0.15→0.16 breaking changes have been addressed. The only outstanding item in the std.c. category is the W.NOHANG fix (Section 6.3, P1).
