...
- [ ] V3.35.S8: P5 — Refactor setup scripts: extract security tool setup out of `*-essential.sh` into a separate `setup-security` command so essential installs stay sudo-minimal and security setup is independently versioned
- [ ] V3.35.S9: P5 — Close the `zig build check` gap: currently only C shims are compile-checked, which misses platform-specific stdlib API breakages in Zig modules (e.g. `std.process.Environ` on Windows); add Zig module compile-check to the check step so cross-platform API drift is caught before test lanes
- [ ] V3.35.S10: P5 — Audit and remove unnecessary dependencies: openssl, rocksdb, snappy — check if they are actually linked or referenced at runtime and remove them from `build.zig` / `justfile` / setup scripts if unused
