# V5.35: Verifiable Security — Making Trust Measurable
...
Security must be measurable, not aspirational. V5.35 transforms Tickoni's security from a
...
executed as no-ops. After V5.35, every lane produces a deterministic pass/fail result
...
V5.35 makes security measurable, not aspirational. Every tool in the recommended
...
- [ ] V5.35.S1: P0 — Fix existing security guarantees: CodeQL, seccomp, CBMC, CODEOWNERS, pin Actions, verify checksums - [story](../stories/v5.35-s1.md)
- [ ] V5.35.S2: P1 — Add static analysis and runtime safety lanes: clang-tidy, OSV-Scanner, zizmor, TSan - [story](../stories/v5.35-s2.md)
- [ ] V5.35.S3: P1 — Add fuzzing infrastructure: libFuzzer targets for C/C++ trust boundaries, Zig built-in fuzzer lane - [story](../stories/v5.35-s3.md)
- [ ] V5.35.S4: P2 — Validate the actual architecture: seccomp/Landlock negative tests, Tickoni property invariant tests, replay determinism testing, expanded CBMC proofs - [story](../stories/v5.35-s4.md)
- [ ] V5.35.S5: P3 — Secure releases: SBOM generation with Syft, artifact CVE scan with Grype, binary hardening verification, OpenSSF Scorecard - [story](../stories/v5.35-s5.md)
- [ ] V5.35.S6: P4 — Experimental depth: CLR non-blocking nightly Zig analysis lane, evidence and documentation closure - [story](../stories/v5.35-s6.md)
- [ ] V5.35.S7: P5 — Story evidence manifest framework: general-purpose build-time verification of roadmap evidence completeness across all epics (V1.x–V3.x), replacing the V2.22.S7 stub and providing reusable infrastructure for all future stories
- [ ] V5.35.S8: P5 — Refactor setup scripts: extract security tool setup out of `*-essential.sh` into a separate `setup-security` command so essential installs stay sudo-minimal and security setup is independently versioned
- [ ] V5.35.S9: P5 — Close the `zig build check` gap: currently only C shims are compile-checked, which misses platform-specific stdlib API breakages in Zig modules (e.g. `std.process.Environ` on Windows); add Zig module compile-check to the check step so cross-platform API drift is caught before test lanes
- [ ] V5.35.S10: P5 — Audit and remove unnecessary dependencies: openssl, rocksdb, snappy — check if they are actually linked or referenced at runtime and remove them from `build.zig` / `justfile` / setup scripts if unused
