# V4.36: Plugin Architecture for Data Sources

**Status:** Backlog
**Milestone:** M4
**Labels:** `epic`, `area/investing`, `area/platform`, `type/feature`

V4.32.S3 described plugin architecture support, but the scope is too large for a single story. V4.36 splits this into its own epic with dedicated stories covering plugin framework, input plugins (ERP, EDGAR), output plugins, discovery/registration, testing, and closure. This epic provides the extensible plugin infrastructure that V4.34 (Valuation Data Layer) consumes for ERP/EDGAR/XBRL data ingestion without code changes.

## Product Intent

Tickoni's valuation engine needs financial data from diverse sources — Excel spreadsheets, SEC EDGAR filings, XBRL data — but hardcoding each source makes new data sources impossible without code changes. V4.36 creates a plugin architecture with a well-defined interface, lifecycle, registry, and discovery system so new data sources can be added by writing a plugin, not by modifying the core runtime. The plugin framework owns input plugins (data source adapters), output plugins (canonical fact producers, validation report producers, audit trail producers), and a plugin discovery/registration system that V4.34 consumes.

## Users And Jobs

- Data engineer: add a new ERP or EDGAR data source by writing a plugin without touching the core runtime.
- Quant researcher: load financial data from multiple sources through a uniform plugin interface.
- Risk reviewer: audit which plugin version, configuration, and source produced each canonical fact.
...
- Input plugins for ERP (Excel/FreeXL) and EDGAR (Edgartools) can be registered and loaded without code changes.
...
- New plugin sources can be added by writing a plugin module without modifying the core plugin framework.
...
- Command or flow: `just plugin-demo` loads an AAPL FY2023 Excel fixture through the ERP plugin, retrieves an EDGAR XBRL fixture through the EDGAR plugin, runs the canonical fact output plugin, and outputs validated canonical facts ready for `tkval`.
- Expected result: deterministic plugin-based data flow with audit trail, source attribution, and plugin version hashes.
...
- [ ] V4.36.S1: Plugin framework — interface, lifecycle hooks, registry, version tracking - #[github-story-issue]
- [ ] V4.36.S2: Input plugins — ERP (Excel/FreeXL) and EDGAR (Edgartools) plugin wrappers - #[github-story-issue]
- [ ] V4.36.S3: Output plugins — canonical fact producer, validation report, audit trail producer - #[github-story-issue]
- [ ] V4.36.S4: Plugin discovery and registration — deterministic loading, config schema, version/hash - #[github-story-issue]
- [ ] V4.36.S5: Plugin test harness — fixtures, replay, audit trail validation, metrics - #[github-story-issue]
- [ ] V4.36.S6: Evidence, demo, and release closure — T10 expansion covering demo, quality gates, docs - #[github-story-issue]
...
- What can the user or operator do now that they could not before this epic?
  Add new data source plugins without code changes to the core runtime.
- What changed from the previous roadmap increment?
  Added an extensible plugin architecture; previous increments hardcoded data sources, this makes them pluggable.
- What is this epic's wow-effect: the visible moment in this epic's demo or workflow that makes Tickoni feel unlike a generic agent harness?
  A new ERP or EDGAR data source added as a single plugin file — no core runtime changes.
