# V4.36: Extensible Data Engine — Plugin-First Financial Data Ingestion

**Status:** Backlog
**Milestone:** M4
**Labels:** `epic`, `area/investing`, `area/platform`, `type/feature`

V4.32.S3 described plugin architecture support, but the scope is too large for a single story. V4.36 splits this into its own epic with dedicated stories covering plugin framework, input plugins (ERP, EDGAR), output plugins, discovery/registration, testing, and closure.

**The why:** Institutions lose weeks and thousands of engineering hours every time they need a new financial data source — Excel from an ERP, SEC EDGAR filings, XBRL data feeds. Each integration used to require modifying the core runtime: code review, deployment, risk assessment. V4.36 removes that bottleneck entirely. With a plugin-first data engine, onboarding a new source is writing a single plugin file. The core runtime never changes.

**The reframe:** Nobody buys "plugin architecture." They buy the ability to start valuing a new company in a day instead of a sprint. This epic turns data integration from a code change into a configuration action.
...
V4.36 eliminates that bottleneck. It creates an extensible plugin infrastructure where new data sources are added by writing a plugin module — not by modifying the core runtime. The plugin framework owns input plugins (data source adapters), output plugins (canonical fact producers, validation report producers, audit trail producers), and a deterministic discovery/registration system. V4.34 (Valuation Data Layer) consumes this infrastructure transparently.
...
- Expected result: a new data source flows through the system end-to-end with full audit trail, source attribution, and plugin version hashes — all from plugin configuration, not core code changes.
