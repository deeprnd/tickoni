...
- **Deletion:** after validation, remove `build-fd.yml`, `build-tk.yml`, `tests-short.yml`, `tests-long.yml`, `tests-xlong.yml`, `quality.yml`, `security.yml`, and the remaining legacy workflow files.
...
- The legacy files consolidated by S4: `build-fd.yml`, `build-tk.yml`, `tests-short.yml`, `tests-long.yml`, `tests-xlong.yml`, `quality.yml`, `security.yml` (7 files), plus the remaining pre-S8 workflow files.
- Files preserved outside consolidation: `coverage.yml`, `benchmark.yml`, `book.yml`, and `ci.yml` (the S8 orchestrator, which is a new file replacing the old ones).
...
- [ ] The old workflow files are deleted after the new `ci.yml` passes validation.
...
- [ ] 7+ legacy workflow files deleted; 3 files (`coverage.yml`, `benchmark.yml`, `book.yml`) unchanged; `ci.yml` orchestrator operational.
