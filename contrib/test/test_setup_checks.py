...
def test_winget_resolution_reports_app_installer_missing():
    with patch.object(winget, '_refresh_winget_path'), \
         patch.object(winget.shutil, 'which', side_effect=lambda name: 'pwsh' if name == 'pwsh' else None), \
         patch.object(winget, '_probe_winget_power_shell', return_value=False), \
         patch.object(winget, '_app_installer_present', return_value=False), \
         patch.object(winget.subprocess, 'run', return_value=SimpleNamespace(returncode=1, stdout='', stderr='')):
        resolution = winget._find_winget_shell()
...
def test_winget_resolution_uses_registered_appx_executable():
    with patch.object(winget, '_refresh_winget_path'), \
         patch.object(winget.shutil, 'which', side_effect=lambda name: 'pwsh' if name == 'pwsh' else None), \
         patch.object(winget, '_appx_winget_path', return_value='C:/WindowsApps/AppInstaller/winget.exe'), \
         patch.object(winget.subprocess, 'run', return_value=SimpleNamespace(
...
def test_winget_path_refresh_does_not_append_entire_package_tree(monkeypatch):
    ...
    path_entries = winget.os.environ['PATH'].split(winget._WINDOWS_PATH_SEP)
...
def test_winget_failure_classifies_missing_package():
    assert winget._winget_failure_status(
        'No package found matching input criteria.'
    ) == 'package_not_found_or_unsupported'
    assert winget._winget_failure_status('network failure') == 'install_failed'
