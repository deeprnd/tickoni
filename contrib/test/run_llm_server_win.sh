...
usage() {
  cat <<'USAGE'
Usage: contrib/test/run_llm_server_win.sh <cpu|gpu>

Runs llama-server.exe with settings tuned for the selected compute backend.
...
if [[ ! -x "$server_bin" ]]; then
  echo "llama-server.exe not found: ${server_bin}" >&2
  echo "Run: just infra-ensure-llamacpp-win" >&2
  exit 1
fi
