...
if (Test-Path $msysBash) {
    ...
    # On ARM64 the MSYS2 package is mingw-w64-aarch64-gcc (UCRT is default runtime).
    if (-not (& $msysBash -lc "gcc --version" 2>$null)) {
        log-info "Installing gcc (mingw-w64-aarch64-gcc) via MSYS2 pacman..."
        $gcc_install = & $msysBash -lc "pacman -S --noconfirm --needed mingw-w64-aarch64-gcc" 2>&1
        if ($LASTEXITCODE -ne 0) {
            log-error "Failed to install mingw-w64-aarch64-gcc: $gcc_install"
            exit 1
        }
    } else {
